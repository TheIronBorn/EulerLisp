...
fn preprocess_fn(
    ...
    match params {
        Datum::Pair(ref ptr) => {
            ...
            for param in elems {
                match param {
                    Datum::Symbol(v) => names.push(v),
                    _ => panic!("Function parameters must be symbols")
                }
            }
        ...
        Datum::Nil => {}
        // TODO: Handle "full-variadic" functions, e.g. (defn foo args (length args))
        ref other => {
            panic!(
    ...
    Ok(Meaning::LambdaDef(names.len(), Box::new(body), dotted))
}
